...
Bu proje, Siber Tehdit İstihbaratı (CTI) araştırmacıları ve analistleri için geliştirilmiş hızlı ve asenkron çalışan web tabanlı bir arama motorudur. Bir Gelişmiş Sürekli Tehdit (APT) grubu adını, diğer ismini (alias) veya MITRE ID'sini kullanarak **MITRE ATT&CK** ve **Malpedia** veritabanlarından eşzamanlı olarak bilgi toplar ve kullanıcı dostu bir arayüzde sunar.
...
- **Kademeli Malpedia Algoritması:** Malpedia'nın doğrudan arama desteklemeyen API'sine karşı, MITRE'den elde edilen alternatif isimleri (synonyms) kullanarak hedef grubu tespit eden özel bir algoritma içerir.
- **Teknik (Technique) Listeleme:** MITRE ATT&CK veritabanındaki saldırı modelleri (attack-patterns) parse edilerek, ilgili APT grubunun kullandığı tüm teknikler (T1059 vb.) listelenir.
